...
interface IconRadioProps extends Omit<SVGProps<SVGSVGElement>, "color"> {
  size?: "default";
  color?: string; // CSS color 값 — style.color 로 적용됨 (fill="currentColor" 상속)
}

export const IconRadio = ({ size, color, className, ...props }: IconRadioProps) => (
  <svg
    width={16}
    height={16}
    className={["icon-radio", size && "size-" + size, className].filter(Boolean).join(" ")}
