...
interface IconUserEditProps extends Omit<SVGProps<SVGSVGElement>, "color"> {
  size?: "default";
  color?: string; // CSS color 값 — style.color 로 적용됨 (fill="currentColor" 상속)
}

export const IconUserEdit = ({ size, color, className, ...props }: IconUserEditProps) => (
  <svg
    width={24}
    height={24}
    className={["icon-user-edit", size && "size-" + size, className].filter(Boolean).join(" ")}
