...
interface IconSearchRefractionProps extends Omit<SVGProps<SVGSVGElement>, "color"> {
  size?: "default";
  color?: string; // CSS color 값 — style.color 로 적용됨 (fill="currentColor" 상속)
}

export const IconSearchRefraction = ({ size, color, className, ...props }: IconSearchRefractionProps) => (
  <svg
    width={24}
    height={24}
    className={["icon-search-refraction", size && "size-" + size, className].filter(Boolean).join(" ")}
