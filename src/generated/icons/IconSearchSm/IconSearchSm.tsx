...
interface IconSearchSmProps extends Omit<SVGProps<SVGSVGElement>, "color"> {
  size?: "default";
  color?: string; // CSS color 값 — style.color 로 적용됨 (fill="currentColor" 상속)
}

export const IconSearchSm = ({ size, color, className, ...props }: IconSearchSmProps) => (
  <svg
    width={24}
    height={24}
    className={["icon-search-sm", size && "size-" + size, className].filter(Boolean).join(" ")}
