...
interface IconArrowBlockUpProps extends Omit<SVGProps<SVGSVGElement>, "color"> {
  size?: "default";
  color?: string; // CSS color 값 — style.color 로 적용됨 (fill="currentColor" 상속)
}

export const IconArrowBlockUp = ({ size, color, className, ...props }: IconArrowBlockUpProps) => (
  <svg
    width={24}
    height={24}
    className={["icon-arrow-block-up", size && "size-" + size, className].filter(Boolean).join(" ")}
