...
interface IconZapOffProps extends Omit<SVGProps<SVGSVGElement>, "color"> {
  size?: "default";
  color?: string; // CSS color 값 — style.color 로 적용됨 (fill="currentColor" 상속)
}

export const IconZapOff = ({ size, color, className, ...props }: IconZapOffProps) => (
  <svg
    width={24}
    height={24}
    className={["icon-zap-off", size && "size-" + size, className].filter(Boolean).join(" ")}
