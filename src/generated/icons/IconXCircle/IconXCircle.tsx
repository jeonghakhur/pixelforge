...
interface IconXCircleProps extends Omit<SVGProps<SVGSVGElement>, "color"> {
  size?: "default";
  color?: string; // CSS color 값 — style.color 로 적용됨 (fill="currentColor" 상속)
}

export const IconXCircle = ({ size, color, className, ...props }: IconXCircleProps) => (
  <svg
    width={24}
    height={24}
    className={["icon-x-circle", size && "size-" + size, className].filter(Boolean).join(" ")}
