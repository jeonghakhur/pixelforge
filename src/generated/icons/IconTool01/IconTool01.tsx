...
interface IconTool01Props extends Omit<SVGProps<SVGSVGElement>, "color"> {
  size?: "default";
  color?: string; // CSS color 값 — style.color 로 적용됨 (fill="currentColor" 상속)
}

export const IconTool01 = ({ size, color, className, ...props }: IconTool01Props) => (
  <svg
    width={24}
    height={24}
    className={["icon-tool01", size && "size-" + size, className].filter(Boolean).join(" ")}
